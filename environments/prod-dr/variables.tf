variable "subscription_id" {
  type        = string
  description = "Azure subscription ID for opslora-prod, which also owns prod-dr resources."
}
...
variable "hub_location_code" {
  type        = string
  description = "Hub short region code for shared DNS resource group names."
  default     = "cin"
}
...
variable "enable_governance_policy_assignments" {
  type        = bool
  description = "Create prod-dr governance policy assignments."
  default     = true
}

variable "prod_dr_private_cluster_enabled" {
  type        = bool
  description = "Whether prod-dr AKS uses a private API server."
  default     = false
}
